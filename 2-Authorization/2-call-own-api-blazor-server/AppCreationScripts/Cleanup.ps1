...
Function Cleanup
{
    ...
    Write-Host "Removing 'client' (call-msgraph-api-blazor) if needed"
    try
    {
        Get-MgApplication -Filter "DisplayName eq 'call-msgraph-api-blazor'" | ForEach-Object {Remove-MgApplication -ApplicationId $_.Id }
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove the application 'call-msgraph-api-blazor'. Error is $message. Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (call-msgraph-api-blazor) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'call-msgraph-api-blazor'" | Format-List Id, DisplayName, AppId, SignInAudience, PublisherDomain
    ...
    {
        Remove-MgApplication -ApplicationId $app.Id
        Write-Host "Removed call-msgraph-api-blazor.."
    }

    # also remove service principals of this app
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'call-msgraph-api-blazor'" | ForEach-Object {Remove-MgServicePrincipal -ServicePrincipalId $_.Id -Confirm:$false}
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove ServicePrincipal 'call-msgraph-api-blazor'. Error is $message. Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
}
