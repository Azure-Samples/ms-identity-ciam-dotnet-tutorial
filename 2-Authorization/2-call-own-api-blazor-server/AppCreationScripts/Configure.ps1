...
Function ConfigureApplications
{
   ...
   Write-Host "Creating the AAD application (call-msgraph-api-blazor)"
   # Get a 6 months application key for the client Application
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInMonths 6
   
   # create the application 
   $clientAadApplication = New-MgApplication -DisplayName "call-msgraph-api-blazor" `
                                                      -Web `
                                                      @{ `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/call-msgraph-api-blazor")
    ...
    Write-Host "Done creating the client application (call-msgraph-api-blazor)"
    ...
    Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>call-msgraph-api-blazor</a></td></tr>" -Path createdApps.html
    ...
    # print the registered app portal URL for any further navigation
    Write-Host "Successfully registered and configured that app registration for 'call-msgraph-api-blazor' at `n $clientPortalUrl" -ForegroundColor Green 
    ...
    $dictionary = @{ "TenantId" = $tenantId;"ClientId" = $clientAadApplication.AppId;"ClientSecret" = $pwdCredential.SecretText;"Scopes" = "api://$($serviceAadApplication.AppId)/ToDoList.Read api://$($serviceAadApplication.AppId)/ToDoList.ReadWrite";"BaseAddress" = $serviceAadApplication.Web.HomePageUrl };
