...
Function Cleanup
{
    ...
    try
    {
        Get-MgApplication -Filter "DisplayName eq 'ciam-daemon-console-v2'" | ForEach-Object {Remove-MgApplication -ApplicationId $_.Id }
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove the application 'ciam-daemon-console-v2'. Error is $message. Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (ciam-daemon-console-v2) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'ciam-daemon-console-v2'" | Format-List Id, DisplayName, AppId, SignInAudience, PublisherDomain
    ...
    {
        Remove-MgApplication -ApplicationId $app.Id
        Write-Host "Removed ciam-daemon-console-v2.."
    }

    # also remove service principals of this app
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'ciam-daemon-console-v2'" | ForEach-Object {Remove-MgServicePrincipal -ServicePrincipalId $_.Id -Confirm:$false}
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove ServicePrincipal 'ciam-daemon-console-v2'. Error is $message. Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
}
