...
Function Cleanup
{
    ...
    Write-Host "Removing 'client' (ciam-dotnet-daemon) if needed"
    try
    {
        Get-MgApplication -Filter "DisplayName eq 'ciam-dotnet-daemon'" | ForEach-Object {Remove-MgApplication -ApplicationId $_.Id }
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove the application 'ciam-dotnet-daemon'. Error is $message. Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (ciam-dotnet-daemon) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'ciam-dotnet-daemon'" | Format-List Id, DisplayName, AppId, SignInAudience, PublisherDomain
    ...
    {
        Remove-MgApplication -ApplicationId $app.Id
        Write-Host "Removed ciam-dotnet-daemon.."
    }

    # also remove service principals of this app
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'ciam-dotnet-daemon'" | ForEach-Object {Remove-MgServicePrincipal -ServicePrincipalId $_.Id -Confirm:$false}
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove ServicePrincipal 'ciam-dotnet-daemon'. Error is $message. Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
}
